...
export default function Edit({ categories, allMenus, promo }) {
  ...
  // preview pakai url gambar lama dari backend
  ...
  const { data, setData, post, processing, errors } = useForm({
    ...
    price: promo.price || '',
    image: null, // file baru (opsional, dipakai hanya saat paket)
    keep_old_image: promo.image_path || null, // path mentah lama
  ...
  const inputClass =
    'w-full bg-white border border-gray-300 rounded-xl px-4 py-2.5 text-sm outline-none transition-all focus:border-gray-500 focus:ring-0 shadow-none font-normal';

  const handleImageChange = (e) => {
    if (!isPackageMode) return; // safety: hanya paket
          ...
          <div className="w-full max-w-2xl px-6 pt-8 pb-12 font-sfPro">
            ...
            <div className="bg-white rounded-[30px] border border-gray-200 shadow-[0_18px_45px_rgba(15,23,42,0.08)]">
              <form onSubmit={handleSubmit} className="p-8 space-y-7">
                ...
                {!isPackageMode && (
                  <div className="bg-blue-50 text-blue-700 p-4 rounded-xl text-sm flex gap-3 items-center border border-blue-100">
                    ...
                    <div>
                      ...
                      <p className="text-xs opacity-80 font-sfPro">
                        Mengubah harga promo untuk menu <strong>{promo.name}</strong>.
                      </p>
                    </div>
                  </div>
                )}
                ...
                {isPackageMode && (
                  <div className="space-y-4">
                    <div className="space-y-1.5">
                      <label className="text-xs text-gray-500 ml-1 tracking-wider font-sfPro">
                        Nama Paket
                      </label>
                      <input
                        type="text"
                ...
                {/* Gambar Promo: hanya untuk paket */}
                {isPackageMode && (
                  <div className="space-y-2">
                    ...
                    <div
                      ...
                      className="w-full border border-gray-300 border-dashed rounded-2xl p-5 flex flex-col items-center justify-center cursor-pointer min-h-[160px] relative overflow-hidden bg-gray-50/40 hover:bg-gray-100 transition-colors"
                    >
                      {preview ? (
                        <>
                          <img
                          ...
                          <div className="absolute inset-0 bg-black/20 flex items-center justify-center">
                            <button
                              type="button"
                              onClick={removeImage}
                              className="bg-black/60 text-white rounded-full px-3 py-1.5 text-[11px] hover:bg-black/80"
                            >
                              Hapus gambar
                            </button>
                          </div>
                ...
                <div className="space-y-2">
                  <label className="block text-sm text-gray-900 font-sfPro">
                    {isPackageMode ? 'Pilih Isi Paket:' : 'Menu yang Didiskon:'}
                  </label>
                    ...
                    {allMenus.map((menu) => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-full flex items-center justify-between px-4 py-3 border-b last:border-b-0 transition-colors ${
                            checked ? 'bg-red-50/40' : 'hover:bg-white'
                          }`}
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div className="text-left">
                              ...
                              <p className="text-[10px] text-gray-400 font-sfPro tracking-tight">
                                Normal: Rp{' '}
                ...
                <div className="bg-gray-50 rounded-2xl p-6 space-y-6 border border-gray-100">
                  <h3 className="text-xs text-gray-400 uppercase tracking-widest font-normal text-center">
                    Periode Aktif Promo
                  </h3>
                  <div className="grid grid-cols-2 gap-8">
                    <div className="space-y-3">
                      <label className="text-[11px] text-gray-400 uppercase block ml-1 font-sfPro">
                        Mulai Berlaku
                      </label>
                      <DatePicker
                        placeholder="Pilih Tanggal"
                        format="YYYY-MM-DD"
                        className="w-full rounded-xl border-gray-300 py-2.5 shadow-none transition-all font-sfPro"
                        value={
                          data.promo_start_date
                            ? dayjs(data.promo_start_date)
                            : null
                        }
                        onChange={(date, dateString) =>
                          setData('promo_start_date', dateString)
                        }
                      />
                      <TimePicker
                        ...
                        value={
                          data.promo_start_time
                            ? dayjs(data.promo_start_time, 'HH:mm')
                            : null
                        }
                        onChange={(time, timeString) =>
                          setData('promo_start_time', timeString)
                        }
                      />
                    </div>
                    <div className="space-y-3">
                      ...
                      <DatePicker
                        ...
                        value={
                          data.promo_end_date
                            ? dayjs(data.promo_end_date)
                            : null
                        }
                        onChange={(date, dateString) =>
                          setData('promo_end_date', dateString)
                        }
                      />
                      <TimePicker
                        placeholder="Pilih Jam"
                        ...
                        value={
                          data.promo_end_time
                            ? dayjs(data.promo_end_time, 'HH:mm')
                            : null
                        }
                        onChange={(time, timeString) =>
                          setData('promo_end_time', timeString)
                        }
                      />
                    </div>
                  </div>
