    ...
    public function index()
    {
        ...
        $grafikBulanan = collect(range(11, 0))->map(function ($i) {
            ...
            return [
                ...
                'pemasukan'   => (float) Transaction::whereYear('created_at', $bulan->year)->whereMonth('created_at', $bulan->month)->sum('total'),
                'pengeluaran' => (float) Product::whereYear('date', $bulan->year)->whereMonth('date', $bulan->month)->sum('price'),
            ];
        })->values();
        ...
        $grafikHarian = collect(range(0, 23))->map(function ($jam) use ($today) {
            return [
                ...
                'pemasukan'   => (float) Transaction::whereDate('created_at', $today)->whereRaw('HOUR(created_at) = ?', [$jam])->sum('total'),
                'pengeluaran' => (float) Product::whereDate('date', $today)->whereRaw('HOUR(created_at) = ?', [$jam])->sum('price'),
            ];
        })->values();
